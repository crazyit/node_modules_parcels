...
function middleware(bundler) {
  //todo\
  console.log("Server middleware bundler.options.outDir:",bundler.options.outDir);
  let fileSplice = bundler.options.outDir.lastIndexOf('\\');
  console.log("outDir:","fileSplice",fileSplice,bundler.options.outDir.substring(0,fileSplice));
  let outDir = bundler.options.outDir.substring(0,fileSplice);
...
async function serve(bundler, port, host, useHTTPS = false) {
  //todo
  // console.log("Server.js -->>bundler",bundler,"useHTTPS:",useHTTPS,":",typeof useHTTPS === 'boolean');
  let handler = middleware(bundler);
  // console.log("Server.js -->>handler",handler);
  let server;
  if (!useHTTPS) {
