...
class Bundler extends EventEmitter {
  constructor(entryFiles, options = {}) {
    super();
    //todo 
    console.log("Bundler constructor() entryFiles1",entryFiles);
    entryFiles = this.normalizeEntries(entryFiles);
    this.watchedGlobs = entryFiles.filter(entry => isGlob(entry));
    //todo 
    console.log("Bundler constructor() entryFiles2",entryFiles);
    this.entryFiles = this.findEntryFiles(entryFiles);
    //todo 
    console.log("Bundler constructor() entryFiles3",this.entryFiles);
  ...
  async bundle() {
    ...
    try {
      ...
      //todo
      console.log("Bundler bundle this.entryFiles:",this.entryFiles);
      ...
      //todo
      logger.success(`Built inSrc ${time}.`);
      if (!this.watcher) {
  ...
  *findOrphanAssets() {
  ...
  async onChange(path) {
    ...
    //todo
    console.log("call cmd:egret sortFile");
    cmdShell.runSync('egret sortFile');
  }
